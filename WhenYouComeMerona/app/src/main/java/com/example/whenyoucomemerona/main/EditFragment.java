...
public class EditFragment extends BaseFragment implements View.OnClickListener, CompoundButton.OnCheckedChangeListener, AdapterView.OnItemClickListener {
    ...
    public EditFragment(AddressTodos todo) {
        this.todo = todo;
    }
    ...
    // 기존 데이터를 가져온다.
    private void setup() {
        etContent.setText(todo.getTodos().getContent());
        etMemo.setText(todo.getTodos().getMemo());
    ...
    private void editItem() {
        params.clear();
        params.put("todo_id", todo.getTodos().getTodo_id() + "");
        ...
        params.put("addr_id", todo.getAddress().getAddr_id()+"");
        ...
        params.put("lat", finalLocation.getLat() + "");
        params.put("lng", finalLocation.getLng() + "");

        params.put("done", todo.getTodos().isDone() + "");
